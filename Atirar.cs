...
public class Atirar : MonoBehaviour {
   ...
   public string TagOutro = "outro";
   public string TagBoss = "boss";
   public string TagInimigoFalso = "inimigoFalso";
   ...
   void Update () {
      ...
      if (Input.GetMouseButtonDown (0) && armas[armaAtual].balasNoPente > 0 && recarregando == false && atirando == false) {
         ...
         if (Physics.Raycast (transform.position, transform.forward, out pontoDeColisao)) {
			...
			if (pontoDeColisao.transform.gameObject.tag == TagOutro) {
               pontoDeColisao.transform.gameObject.GetComponent<Outro> ().vida -= armas[armaAtual].danoPorTiro;
            }
			 if (pontoDeColisao.transform.gameObject.tag == TagInimigoFalso) {
               pontoDeColisao.transform.gameObject.GetComponent<InimigoFalso> ().vida -= armas[armaAtual].danoPorTiro;
            }
			if (pontoDeColisao.transform.gameObject.tag == TagBoss) {
               pontoDeColisao.transform.gameObject.GetComponent<Boss> ().vida -= armas[armaAtual].danoPorTiro;
            }
         }
      }
      //recarregar
